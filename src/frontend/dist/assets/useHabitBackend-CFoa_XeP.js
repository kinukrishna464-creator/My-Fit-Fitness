...
const ReadingEntry = Record({
  "minutes": Nat,
  "category": Text,
  "pages": Nat
});
const MeditationSession = Record({
  "id": Text,
  "durationSeconds": Nat
});
const Task = Record({
...
Service({
  ...
  "getMoodByDate": Func([Text], [Opt(Nat)], ["query"]),
  "getReadingByDate": Func([Text], [Vec(ReadingEntry)], ["query"]),
  "getTasksByDate": Func([Text], [Vec(Task)], ["query"]),
  "getWorkoutsByDate": Func(
    [Text],
    [Vec(WorkoutEntry)],
    ["query"]
  ),
...
const idlFactory = ({ IDL: IDL2 }) => {
  const ReadingEntry2 = IDL2.Record({
    "minutes": IDL2.Nat,
    "category": IDL2.Text,
    "pages": IDL2.Nat
  });
  const MeditationSession2 = IDL2.Record({
    "id": IDL2.Text,
  ...
  const WorkoutEntry2 = IDL2.Record({
    "reps": IDL2.Nat,
    "sets": IDL2.Nat,
    "category": IDL2.Text
  });
  const DaySummary2 = IDL2.Record({
    "reading": IDL2.Vec(ReadingEntry2),
    "meditation": IDL2.Vec(MeditationSession2),
    "tasks": IDL2.Vec(Task2),
    "date": IDL2.Text,
  ...
  return IDL2.Service({
    ...
    "getMoodByDate": IDL2.Func([IDL2.Text], [IDL2.Opt(IDL2.Nat)], ["query"]),
    "getReadingByDate": IDL2.Func(
      [IDL2.Text],
      [IDL2.Vec(ReadingEntry2)],
      ["query"]
    ),
    "getTasksByDate": IDL2.Func([IDL2.Text], [IDL2.Vec(Task2)], ["query"]),
    "getWorkoutsByDate": IDL2.Func(
      [IDL2.Text],
...
class Backend {
  ...
  async getMeditationByDate(arg0) {
    if (this.processError) {
      try {
  ...
  async removeMeditationSession(arg0, arg1) {
    if (this.processError) {
      try {
  ...
  async removeReadingEntry(arg0, arg1) {
    if (this.processError) {
      try {
  ...
  async removeWorkoutEntry(arg0, arg1) {
    if (this.processError) {
      try {
  ...
  async upsertReadingEntry(arg0, arg1, arg2, arg3) {
    if (this.processError) {
      try {
  ...
  async upsertWorkoutEntry(arg0, arg1, arg2, arg3) {
    if (this.processError) {
      try {
...
function from_candid_DaySummary_n1(_uploadFile, _downloadFile, value) {
...
const keys = {
  daySummary: (date) => ["daySummary", date],
  previousDay: (date) => ["previousDay", date],
  workout: (date) => ["workout", date],
  hydration: (date) => ["hydration", date],
  mood: (date) => ["mood", date],
  tasks: (date) => ["tasks", date],
  reading: (date) => ["reading", date],
  meditation: (date) => ["meditation", date]
};
function usePreviousDaySummary(currentDate = todayKey()) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return actor.getDailySummary(prevKey);
    },
    enabled: !!actor && !isFetching
  });
}
function useWorkoutLog(date = todayKey()) {
  const { actor, isFetching } = useActor(createActor);
  return useQuery({
    queryKey: keys.workout(date),
    queryFn: async () => {
      if (!actor) return [];
      return actor.getWorkoutsByDate(date);
    },
    enabled: !!actor && !isFetching
  });
}
function useSaveWorkout(date = todayKey()) {
...
function useMood(date = todayKey()) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (!actor) return null;
      return actor.getMoodByDate(date);
    },
    enabled: !!actor && !isFetching
  });
}
function useSaveMood(date = todayKey()) {
  const { actor } = useActor(createActor);
  const qc = useQueryClient();
  return useMutation({
...
function useTasks(date = todayKey()) {
  const { actor, isFetching } = useActor(createActor);
  return useQuery({
...
function useAddTask(date = todayKey()) {
  const { actor } = useActor(createActor);
  const qc = useQueryClient();
  return useMutation({
...
function useToggleTask(date = todayKey()) {
  const { actor } = useActor(createActor);
  const qc = useQueryClient();
  return useMutation({
...
function useDeleteTask(date = todayKey()) {
  const { actor } = useActor(createActor);
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (taskId) => {
...
function useReadingLog(date = todayKey()) {
  const { actor, isFetching } = useActor(createActor);
  return useQuery({
    queryKey: keys.reading(date),
    queryFn: async () => {
      if (!actor) return [];
      return actor.getReadingByDate(date);
    },
    enabled: !!actor && !isFetching
  });
}
function useSaveReading(date = todayKey()) {
  const { actor } = useActor(createActor);
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (entry) => {
    ...
    onSuccess: () => void qc.invalidateQueries({ queryKey: keys.reading(date) })
  });
}
function useMeditationLog(date = todayKey()) {
  const { actor, isFetching } = useActor(createActor);
  return useQuery({
    queryKey: keys.meditation(date),
    queryFn: async () => {
      if (!actor) return [];
      return actor.getMeditationByDate(date);
    },
    enabled: !!actor && !isFetching
  });
}
function useAddMeditation(date = todayKey()) {
  const { actor } = useActor(createActor);
  const qc = useQueryClient();
  return useMutation({
    ...
    onSuccess: () => void qc.invalidateQueries({ queryKey: keys.meditation(date) })
  });
}
function useDeleteMeditation(date = todayKey()) {
  const { actor } = useActor(createActor);
  const qc = useQueryClient();
  return useMutation({
